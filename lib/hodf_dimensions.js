...
function to_hot_properties(prop) {
    ...
    if (prop.type === 'dropdown') {
        return {type: 'dropdown', source: prop.source};
    }
    return {};
}
...
ListDimension.prototype.headerHTML = function () { return this.values.map(function (x) { return x.title || x.name; }); };
...
RangeDimension.prototype.headerHTML = function () {
    return this.initial.map(function (x) { return (x.title || x.name); }).concat(
...
RangeDimension.prototype.dataProperties = function () {
    return this.initial.map(to_hot_properties).concat(
        sequence(this.min, this.max).map(function (x) { return to_hot_properties(this.prefix); }.bind(this))
    );
};
RangeDimension.prototype.update = function (paramEl, hot, e) {
    ...
    function to_num(x, offset) {
        return (parseInt(x.substr((self.prefix.title || self.prefix.name).length), 10) + (offset || 0)).toString();
    }
    ...
    for (i = 0; i < 2000; i++) {
        if (to_num(oldHeaders[this.initial.length]) > to_num(newHeaders[this.initial.length])) {
            ...
            oldHeaders.splice(this.prefix.title + this.initial.length, 0, to_num(oldHeaders[this.initial.length], -1));
            ...
            oldHeaders.push(this.prefix.title + to_num(oldHeaders[oldHeaders.length - 1], 1));
...
function get_dimension(t, init_headings) {
    if (Array.isArray(t)) {
        return new ListDimension({values: t}, init_headings);
    }

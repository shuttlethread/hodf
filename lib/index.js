...
function Hodataframe(tmpl, el, inital_data, initial_lang) {
    ...
    hotParams.maxRows = hotParams.rowHeaders.length;  // Stop c+p tampering with table size
    hotParams.colHeaders = cols.headerHTML(initial_lang);
    hotParams.fillHandle = { autoInsertRow: false };  // Stop autofill tampering with table size
    hotParams.startCols = hotParams.colHeaders.length;
    hotParams.maxCols = hotParams.colHeaders.length;  // Stop c+p tampering with table size
    ...
    el.querySelector(".parameters").addEventListener('change', function (e) {
        ...
        hot.updateSettings({
            ...
            maxRows: rows.headers().length,
            colHeaders: cols.headerHTML(initial_lang),
            maxCols: cols.headers().length,

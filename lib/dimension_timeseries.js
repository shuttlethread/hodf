...
TimeSeriesDimension.prototype.parameterHtml = function (lang) {
    return [
        ...
        '</label>&nbsp;',
        this.allowed_periods.length > 1 ? '<select name="delta">\n' + this.allowed_periods.map(function (p) {
            // TODO: We should have multiple select boxes instead, and only update from whatever is in target_el
...
TimeSeriesDimension.prototype.update = function (param_el, target_el) {
    var out = [],
        ...
        delta_el = param_el.querySelector("select[name=delta]") || { options: [{ value: this.delta, selected: true }] },
